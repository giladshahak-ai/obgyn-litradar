...
# כל פריט: (תווית, שאילתת PubMed, קטגוריה — אחת מ-6 הנושאים). הקטגוריה לסינון במגמות.
TREND_TOPICS = [
    ("GLP-1 / סמגלוטייד בהריון ו-PCOS", '(semaglutide[tiab] OR "GLP-1"[tiab] OR tirzepatide[tiab]) AND (pregnan*[tiab] OR "polycystic ovary"[tiab] OR fertilit*[tiab])', "אנדוקרינולוגיה"),
    ("השראת לידה בשבוע 39", '"induction of labor"[tiab] AND ("39 weeks"[tiab] OR "elective induction"[tiab])', "מיילדות"),
    ("השראת לידה במאקרוזומיה", '"induction of labor"[tiab] AND (macrosomia[tiab] OR "large for gestational age"[tiab] OR "suspected macrosomia"[tiab])', "מיילדות"),
    ("טכניקת סגירת הרחם בקיסרי", '(cesarean[tiab] OR caesarean[tiab]) AND (("uterine closure"[tiab] OR "double layer"[tiab] OR "single layer"[tiab]) OR niche[tiab] OR isthmocele[tiab])', "מיילדות"),
    ("חומצה טרנקסמית לדימום אחרי לידה", '"tranexamic acid"[tiab] AND ("postpartum hemorrhage"[tiab] OR "postpartum haemorrhage"[tiab])', "מיילדות"),
    ("אספירין למניעת רעלת הריון", '("low-dose aspirin"[tiab] OR aspirin[tiab]) AND (preeclampsia[tiab] OR "pre-eclampsia[tiab]")', "מיילדות"),
    ("PGT-A — בדיקה גנטית טרום-השרשה", '("preimplantation genetic testing"[tiab] OR "PGT-A"[tiab] OR aneuploidy[tiab]) AND (embryo[tiab] OR IVF[tiab])', "פוריות"),
    ("טיפול תרופתי באנדומטריוזיס (GnRH)", 'endometriosis[tiab] AND (elagolix[tiab] OR relugolix[tiab] OR "GnRH antagonist"[tiab] OR dienogest[tiab])', "אנדומטריוזיס"),
    ("דגימה עצמית ל-HPV", '("self-sampling"[tiab] OR "self-collection"[tiab]) AND (HPV[tiab] OR "cervical cancer"[tiab])', "אונקוגינקולוגיה"),
    ("ניתוח עוברי תוך-רחמי", '("fetal surgery"[tiab] OR "in utero"[tiab] OR "intrauterine surgery"[tiab]) AND (spina bifida[tiab] OR myelomeningocele[tiab] OR fetoscop*[tiab])', "מיילדות"),
    ("פרוגסטרון למניעת לידה מוקדמת", 'progesterone[tiab] AND ("preterm birth"[tiab] OR "preterm labor"[tiab])', "מיילדות"),
    ("השתלת רחם", '"uterus transplantation"[tiab] OR "uterine transplantation"[tiab]', "פוריות"),
    ("בינה מלאכותית באולטרסאונד מיילדותי", '("artificial intelligence"[tiab] OR "deep learning"[tiab] OR "machine learning"[tiab]) AND (ultrasound[tiab] OR fetal[tiab] OR obstetric[tiab])', "מיילדות"),
    ("NIPT / cfDNA מורחב", '("cell-free DNA"[tiab] OR "cfDNA"[tiab] OR "noninvasive prenatal"[tiab]) AND (screening[tiab] OR microdeletion[tiab])', "מיילדות"),
    ("מטפורמין בסוכרת הריון", 'metformin[tiab] AND ("gestational diabetes"[tiab] OR pregnancy[tiab])', "מיילדות"),
    ("בחירת עוברים מבוססת AI / time-lapse", '("time-lapse"[tiab] OR "artificial intelligence"[tiab]) AND (embryo[tiab] OR blastocyst[tiab])', "פוריות"),
    ("העברת עובר מוקפא מול טרי", '("frozen embryo transfer"[tiab] OR "freeze-all"[tiab]) AND (live birth[tiab] OR pregnancy rate[tiab])', "פוריות"),
    ("מגנזיום להגנה נוירולוגית עוברית", '"magnesium sulfate"[tiab] AND (neuroprotection[tiab] OR "cerebral palsy"[tiab] OR preterm[tiab])', "מיילדות"),
    ("פסר/תפר צווארי", '(cerclage[tiab] OR pessary[tiab]) AND ("preterm birth"[tiab] OR "cervical insufficiency"[tiab] OR "short cervix"[tiab])', "מיילדות"),
    ("לטרוזול להשראת ביוץ", 'letrozole[tiab] AND (ovulation[tiab] OR "polycystic ovary"[tiab] OR infertility[tiab])', "פוריות"),
    ("מיקרוביום נרתיקי ובריאות רבייה", 'microbiome[tiab] AND (vaginal[tiab] OR endometri*[tiab] OR pregnancy[tiab])', "גינקולוגיה כללית"),
    ("תמיכת דולה בלידה", 'doula[tiab] AND (birth[tiab] OR labor[tiab] OR delivery[tiab])', "מיילדות"),
    ("רלוגוליקס לשרירנים", 'relugolix[tiab] AND (fibroid[tiab] OR leiomyoma[tiab] OR "uterine bleeding"[tiab])', "גינקולוגיה כללית"),
    ("הידוק מאוחר של חבל הטבור", '("delayed cord clamping"[tiab] OR "umbilical cord milking"[tiab])', "מיילדות"),
    ("דיאטה ים-תיכונית בהריון", '"mediterranean diet"[tiab] AND (pregnan*[tiab] OR "gestational"[tiab])', "מיילדות"),
    ("אימונותרפיה בסרטן שחלה/רחם", '(pembrolizumab[tiab] OR immunotherapy[tiab] OR "checkpoint inhibitor"[tiab]) AND (ovarian[tiab] OR endometrial[tiab] OR cervical[tiab])', "אונקוגינקולוגיה"),
]
...
# ── ציון חשיבות (0..100) — משקלים מסתגלים לפי גיל המאמר ─────────────────
# העיקרון (EBM + ביבליומטריקה): ציטוטים חסרי-משמעות עד שהמאמר "מתבגר".
# אלה משקלי-בסיס למאמר בוגר. למאמר טרי, משקל ה"השפעה" יורד ל-0 והמשקל עובר
# לרמת הראיות (העוגן האובייקטיבי הטוב ביותר בזמן הפרסום). ראה _maturity.
SCORE_WEIGHTS = {
    "design":  0.30,   # רמת ראיות — מטא/RCT > תצפיתי > דעה (חזק במיוחד למאמר טרי)
    "journal": 0.15,   # השפעת העיתון (אות משני)
    "author":  0.15,   # מעמד החוקרים (h-index — אות משני, ביקורתי)
    "impact":  0.40,   # השפעה בפועל (FWCI/ציטוטים) — רק כשהמאמר מתבגר
}
AUTHOR_HINDEX_TOP = 50    # h-index שמעליו = ציון מלא
IMPACT_FWCI_TOP   = 20    # FWCI שמעליו = השפעה מלאה (פי-20 מממוצע התחום)
# חלון התבגרות ציטוטים: <6 ח' → משקל השפעה 0; גדל לינארית עד מלא ב-24 ח'.
CITATION_MIN_MONTHS    = 6
CITATION_MATURE_MONTHS = 24
