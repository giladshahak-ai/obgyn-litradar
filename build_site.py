...
.jbrand{display:inline-block;font-family:'Frank Ruhl Libre',serif;font-weight:900;color:#fff;border-radius:9px;padding:5px 14px;font-size:15.5px;letter-spacing:.3px;box-shadow:0 2px 7px rgba(0,0,0,.13);margin-bottom:9px;line-height:1.2}
...
function specialBadges(a){
  ...
  return s;}
// צבעי מותג מזוהים לכל עיתון — לוורדמרק הבולט ולפס בשולי הכרטיס
const JBRAND={'NEJM':'#C8102E','Lancet':'#6E1A1A','JAMA':'#003A70',
  'AJOG':'#6C2D6B','Green Journal':'#1F7A3D','BJOG':'#00529B',
  'Ultrasound O&G':'#0E7C7B','AJOG MFM':'#8E44AD','Gyn Oncology':'#B23A48',
  'Fertility & Sterility':'#C77400'};
function jBrand(n){return JBRAND[n]||'#5E584E';}
function cardHTML(a,i){
  ...
  return `<div class="card${j.tier===1?' tier1':''}" style="border-right:6px solid ${jBrand(j.nick)}">
    <div class="head">
      ...
      <div style="flex:1">
        <div class="jbrand" style="background:${jBrand(j.nick)}">${j.tier===1?'👑 ':''}${esc(j.nick)}</div>
