...
// מגביל מקביליות (ולא טורי) — מהיר בהרבה תוך כיבוד מגבלות הקצב
function makeLimiter(max){let active=0;const q=[];
  const run=()=>{while(active<max&&q.length){const it=q.shift();active++;Promise.resolve().then(it.fn).then(it.res,it.rej).finally(()=>{active--;run();});}};
  return fn=>new Promise((res,rej)=>{q.push({fn,res,rej});run();});}
const ncbiGate=makeLimiter(3);  // עד 3 בקשות במקביל (מגבלת NCBI)
const oaGate=makeLimiter(5);
async function tfetch(url,opt,ms){const c=new AbortController();const t=setTimeout(()=>c.abort(),ms||22000);
  try{return await fetch(url,Object.assign({signal:c.signal},opt||{}));}finally{clearTimeout(t);}}

async function withRetry(fn,n){for(let i=0;i<(n||3);i++){try{return await fn();}catch(e){if(i===(n||3)-1)throw e;await new Promise(s=>setTimeout(s,600*(i+1)));}}}
async function eutilsJSON(url){return ncbiGate(()=>withRetry(()=>tfetch(url).then(r=>r.json())));}
async function eutilsText(url){return ncbiGate(()=>withRetry(()=>tfetch(url).then(r=>r.text())));}
...
// ---- PubMed search ----
// שאילתה אחת לקבוצת עיתונים (כולם הומוגניים ב-filter_obgyn) — חוסך בקשות
function groupTerm(js,fullIssue){
  const issns=js.map(j=>`${j.issn_e}[IS] OR ${j.issn}[IS]`).join(' OR ');
  let t=`(${issns})`;
  if(js[0].filter_obgyn) t+=` AND ${CFG.obgyn_filter}`;
  if(!fullIssue) t+=` AND ${CFG.high_value_filter}`;
  return t;
}
...
async function runSearch(){
  ...
  // מקבצים את העיתונים לקבוצות הומוגניות (ייעודי / כללי) → 1-2 שאילתות בלבד
  let groups;
  if(state.activeJournal){const j=CFG.journals.find(x=>x.nick===state.activeJournal);groups=j?[[j]]:[];}
  else{const spec=CFG.journals.filter(j=>!j.filter_obgyn),gen=CFG.journals.filter(j=>j.filter_obgyn);groups=[spec,gen].filter(g=>g.length);}
  setStatus((fi?'טוען את הגיליון המלא':'סורק את PubMed בזמן אמת')+'… <span class="spinner"></span>');
  document.getElementById('feed').innerHTML='';
  try{
    let done=0;const N=groups.length;
    const idLists=await Promise.all(groups.map(g=>esearch(groupTerm(g,fi), fi?300:250)
      .then(r=>{done++;setStatus(`סורק… ${done}/${N} <span class="spinner"></span>`);return r;})
      .catch(()=>{done++;return [];})));
    setStatus('טוען מאמרים… <span class="spinner"></span>');
    ...
    <div class="head">
      ...
      <div style="flex:1">
        ...
        <div class="actions">
          ...
          <button class="btn" onclick="toNotebookLM('${a.pmid}','${esc(a.doi||'')}')" title="העתק קישור ופתח NotebookLM">📓 NotebookLM</button>
        </div>
...
// ---- NotebookLM ----
function toast(msg){let t=document.getElementById('lrtoast');if(!t){t=document.createElement('div');t.id='lrtoast';t.style.cssText='position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#2A2722;color:#fff;padding:11px 18px;border-radius:12px;font-size:14px;z-index:200;box-shadow:0 8px 24px rgba(0,0,0,.25);opacity:0;transition:.25s;max-width:90%;text-align:center';document.body.appendChild(t);}t.textContent=msg;requestAnimationFrame(()=>t.style.opacity='1');clearTimeout(t._t);t._t=setTimeout(()=>t.style.opacity='0',4500);}
function toNotebookLM(pmid,doi){
  const url=doi?('https://doi.org/'+doi):('https://pubmed.ncbi.nlm.nih.gov/'+pmid+'/');
  const done=()=>{toast('הקישור הועתק ✓ — ב-NotebookLM: Add source → Website → הדבק (Ctrl+V)');window.open('https://notebooklm.google.com/','_blank');};
